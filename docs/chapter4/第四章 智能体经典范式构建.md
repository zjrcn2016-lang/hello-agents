...
针对本节设定的目标——回答关于”华为最新手机”的问题，我们需要为智能体提供一个网页搜索工具。在这里我们选用 <strong>Tavily</strong>，它是一个专为 AI 应用设计的搜索 API，能够直接返回经过提炼的答案摘要，非常适合作为智能体的工具。
...
同时，你需要前往 [Tavily官网](https://tavily.com/) 注册一个免费账户，获取你的API密钥。该密钥已在 4.1.2 节的 `.env` 文件中配置：
...
在上述代码中，首先会检查 Tavily 是否返回了综合后的直接答案（`answer` 字段）。如果存在，就直接返回这个最精确的答案。如果不存在，它才会退而求其次，返回前三条搜索结果的内容摘要。这种”智能解析”能为LLM提供质量更高的信息输入。
...
🔍 正在执行 [Tavily] 网页搜索: 华为最新手机型号及主要卖点
👀 观察: [1] 华为手机- 华为官网
